...
<!-- 选项卡开始 -->
 ...
 <div id="human">
   <table cellspacing="12">
     <tr><input type="hidden" name="id" value="${requestScope.contribute.resource_id }"/>
       <td align="center"> 
      <c:choose>   
            <c:when test="${requestScope.contribute.status==1}">已接受</c:when>
            <c:otherwise>未接受</c:otherwise>
        </c:choose>
     </td>
     
     <c:forEach items="${userList}" var="user" >
       <c:if test="${user.user_id==requestScope.contribute.resource_id}">
 ...
 <table cellspacing="12">
    <tr>
       <td align="left">
       <h2><strong> ${requestScope.contribute.resource_title}</strong></h2>
         </td>
       
    </tr>
    <tr>
    <td>${requestScope.contribute.resource_describe }</td>
    </tr>
 ...
 <div class="banben">
  ...
  <div id="content">
       ...
       <td align="left">
       <h3><strong> ${requestScope.contribute.resource_title}</strong></h3>
         </td>
         <td>${requestScope.contribute.resource_label}</td>
    
    <textarea name="resource_content">
     ${requestScope.contribute.resource_content}
   </textarea>
      <c:if test="${not empty requestScope.contribute.resource_describe}">
             <p> 资源描述：
              <br/>
               ${requestScope.contribute.resource_describe}
              </p> 
       </c:if>
  <c:if test="${not empty requestScope.contribute.resource_filePath}">
      <p> 附件列表：
              <br/>
               ${requestScope.contribute.resource_filePath}
              </p> 
  </c:if>
  </div>
  <!-- 更新版本 -->
